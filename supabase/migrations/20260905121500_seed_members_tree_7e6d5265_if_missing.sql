-- Bổ sung dữ liệu mẫu cho cây 7e6d5265-… khi chưa có bộ seed (nhận biết qua ghi chú gốc).
DO $$
DECLARE
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.family_trees WHERE id = tid) THEN
    RAISE NOTICE 'seed_tree_7e6d5265_v2: không có family_trees id % — bỏ qua', tid;
    RETURN;
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE family_tree_id = tid
      AND notes = 'Thế hệ gốc (dữ liệu mẫu)'
    LIMIT 1
  ) THEN
    RAISE NOTICE 'seed_tree_7e6d5265_v2: bộ mẫu đã có trên cây % — bỏ qua', tid;
    RETURN;
  END IF;
  ...
  VALUES (
    ...
    'male',
    '1952-06-01',
    'Con trai ông bà Tổ (dữ liệu mẫu)',
    m1,
    m2
  )
  ...
  RAISE NOTICE 'seed_tree_7e6d5265_v2: đã thêm 5 thành viên mẫu cho cây %', tid;
END $$;
