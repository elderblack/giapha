...
import { type FeedReactionKind, parseReactionKind } from './reactionKinds'
...
export async function loadFeedPostById(postId: string): Promise<FeedPostState | null> {
  const sb = getSupabase()
  if (!sb) return null
  ...
  const hydrated = await hydrateFeedPosts([row as BasePostRow])
  return hydrated[0] ?? null
}

/** Bài của tài khoản (RLS). Phân trang offset giống web. */
export async function loadMyPostsPage(
  ...
  const treeIds = [...new Set(hydrated.map((x) => x.family_tree_id))]
  ...
  return hydrated.map((x) => ({
    ...x,
    tree_name: nameById.get(x.family_tree_id) ?? null,
  }))
}

/** Tải lại bài theo id (sau reaction / xoá / v.v.). */
export async function reloadFeedPostsByIds(postIds: string[]): Promise<Map<string, FeedPostState>> {
  ...
  const withTrees = hydrated.map((x) => ({
    ...x,
    tree_name: nameById.get(x.family_tree_id) ?? null,
  }))
  for (const item of withTrees) map.set(item.id, item)
  return map
}

/** Dòng họ user có vai trò — chọn khi đăng từ hồ sơ. */
